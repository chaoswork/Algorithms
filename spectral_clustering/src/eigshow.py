...
def redrawPlot(event):
    ...
    and re-draws it on the canvas. Since the canvas is not changed,
    the visibility of older lines are set to false
    """
    global line1old, line2old, line3old, line4old
    global text1old, text2old, text3old, text4old
    global svdVis, bSVD, redrawCount
    # returns x_0,y_0,x_1,y_1 for the axes in pixels
    t = ax.get_window_extent().extents
    # mouse click within unit circle
    # mouse click within the axes (necessary)
    if ((-1 <= event.xdata <= 1) and (-1 <= event.ydata <= 1) and
            (t[0] <= event.x <= t[2]) and (t[1] <= event.y <= t[3])):
        ...
        ax.scatter(x[0, 0], x[1, 0], c=u'r', marker='o', s=18, alpha=1.0,
                   zorder=zorder_r)  # x
        ax.scatter(Axnew[0, 0], Axnew[1, 0], c=u'b', marker='o', s=18,
                   alpha=1.0, zorder=zorder_b)  # Ax
        if bSVD:
            ax.scatter(y[0, 0], y[1, 0], c=u'r', marker='o', s=18,
                       alpha=1.0, zorder=zorder_r)  # y
            ax.scatter(Aynew[0, 0], Aynew[1, 0], c=u'b', marker='o', s=18,
                       alpha=1.0, zorder=zorder_b)  # Ay

        # erase the old lines
        line1old.set_visible(False)
        line2old.set_visible(False)
        text1old.set_visible(False)
        text2old.set_visible(False)
        line3old.set_visible(False)
        line4old.set_visible(False)
        text3old.set_visible(False)
        text4old.set_visible(False)

        # draw new lines -- x and Ax
        line1new, = ax.plot([0.0, x[0, 0]], [0.0, x[1, 0]], c='r', aa=True)
        line2new, = ax.plot([0.0, Axnew[0, 0]], [0.0, Axnew[1, 0]],
                            c='b', aa=True)
        text1new = ax.text((0.0 + 0.8 * tlr * x[0, 0]),
                           (0.0 + 0.8 * tlr * x[1, 0]),
                           '$x$', fontsize=15, color='r',
                           bbox=dict(facecolor='white',
                                     edgecolor='white',
                                     alpha=0.5))
        text2new = ax.text((0.0 + tlr * Axnew[0, 0]),
                           (0.0 + tlr * Axnew[1, 0]),
                           '$Ax$', fontsize=15, color='b',
                           bbox=dict(facecolor='white',
                                     edgecolor='white',
                                     alpha=0.5))
        ...
        line4new, = ax.plot([0.0, Aynew[0, 0]], [0.0, Aynew[1, 0]],
                            c='g', aa=True)
        text3new = ax.text((0.0 + 0.8 * tlr * y[0, 0]),
                           (0.0 + 0.8 * tlr * y[1, 0]),
                           '$y$', fontsize=15, color='m',
                           bbox=dict(facecolor='white',
                                     edgecolor='white',
                                     alpha=0.5))
        text4new = ax.text((0.0 + tlr * Aynew[0, 0]),
                           (0.0 + tlr * Aynew[1, 0]),
                           '$Ay$', fontsize=15, color='g',
                           bbox=dict(facecolor='white',
                                     edgecolor='white', alpha=0.5))
        line3new.set_visible(svdVis)
        line4new.set_visible(svdVis)
        text3new.set_visible(svdVis)
        text4new.set_visible(svdVis)
...
def drawPlot():
    ...
    # (w is make a global as it is used in resetAxis())
    global A, fig, ax, root, w
    ...
    # w contains the eigen values, v contains the eigen vectors
    w, v = np.linalg.eig(A)
    resetAxes()
    ...
    # (it will not be seen initially as the visibility is false)
    ...
    text1 = ax.text((0.0 + 0.8 * tlr * xstart[0, 0]),
                    (0.0 + 0.8 * tlr * xstart[1, 0]),
                    '$x$', fontsize=15, color='r',
                    bbox=dict(facecolor='white', edgecolor='white', alpha=0.5))
    text2 = ax.text((0.0 + tlr * Axstart[0, 0]),
                    (0.0 + tlr * Axstart[1, 0]), '$Ax$',
                    fontsize=15, color='b',
                    bbox=dict(facecolor='white', edgecolor='white', alpha=0.5))
    ...
    line3, = ax.plot([0.0, ystart[0, 0]], [0.0, ystart[1, 0]], aa=True, c='m',
                     visible=svdVis)
    line4, = ax.plot([0.0, Aystart[0, 0]], [0.0, Aystart[1, 0]], aa=True,
                     c='g', visible=svdVis)
    text3 = ax.text((0.0 + 0.8 * tlr * ystart[0, 0]),
                    (0.0 + 0.8 * tlr * ystart[1, 0]),
                    '$y$', fontsize=15, color='m',
                    bbox=dict(facecolor='white',
                              edgecolor='white',
                              alpha=0.5),
                    visible=svdVis)
    text4 = ax.text((0.0 + tlr * Aystart[0, 0]),
                    (0.0 + tlr * Aystart[1, 0]), '$Ay$',
                    fontsize=15, color='g',
                    bbox=dict(facecolor='white',
                              edgecolor='white',
                              alpha=0.5),
    ...
    if complexEigenVals and not bSVD:
        comValTxt = ax.text(0 - 0.5*ax.get_xlim()[1], 0.5*ax.get_ylim()[1],
                            "Complex eigen values", ha='left', color='y',
                            fontsize='large', fontweight='bold', alpha=0.4)
    if complexEigenVecs and not bSVD:
        comVecTxt = ax.text(0 - 0.5*ax.get_xlim()[1], 0.4*ax.get_ylim()[1],
                            "Complex eigen vectors", ha='left', color='y',
                            fontsize='large', fontweight='bold', alpha=0.4)
    if complexSingVals and bSVD:
        comValTxt = ax.text(0 - 0.5*ax.get_xlim()[1], 0.5*ax.get_ylim()[1],
                            "Complex singular values", ha='left', color='y',
                            fontsize='large', fontweight='bold', alpha=0.4)
    if complexSingVecs and bSVD:
        comVecTxt = ax.text(0 - 0.5*ax.get_xlim()[1], 0.4*ax.get_ylim()[1],
                            "Complex singular vectors", ha='left', color='y',
                            fontsize='large', fontweight='bold', alpha=0.4)
    ...
    indTxt = fig.text(0.3, 0.03, indStr, ha='left', color='g',
...
if __name__ == '__main__':
    eigshow(np.matrix([[1.0/4, 3.0/4], [4.0/4, 2.0/4]]))
